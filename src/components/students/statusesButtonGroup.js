...
const StatusesButton = ({statuses, selectedStatuses, statuseStudents}) => {
    ...
    return(
        <>
            <Col>
                <Button
                    style={{ marginBottom: "2px", width:"100%", wordWrap: "break-word",  textAlign: "center"}}
                    color="dark"
                    onClick={() => {onRadioBtnClick("all"); statuseStudents(undefined, undefined, "all") }}
            ...
            statuses.sort(function(a,b){return a.sort - b.sort}).map(status => (
                <Col key={status.id}>
                    <Button
                        ...
                        style={{ marginBottom: "2px", width:"100%", backgroundColor: status.color, borderColor: status.color,  wordWrap: "break-word", textAlign: "center" }}
                        color="dark"
                        id={status.id}
                        key={status.id}
                        onClick={() => {onRadioBtnClick(status.id); statuseStudents(undefined, undefined, status) }}
